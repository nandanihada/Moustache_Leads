...
class LoginLogsService {
  ...
  async getScheduledActivity(userId?: string): Promise<any> {

    const url = userId ? `/api/admin/scheduled-activity?user_id=${userId}` : '/api/admin/scheduled-activity';
  ...
  async getUserAutomations(userId: string): Promise<any> {
    const response = await api.get(`/api/admin/user-automations/${userId}`);
    return response.data;
  }

  async saveUserAutomations(userId: string, data: any): Promise<any> {
    const response = await api.post(`/api/admin/user-automations/${userId}`, data);
    return response.data;
  }
}
