                            ...
                            <div className="flex items-start gap-2">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-0.5 text-xs text-muted-foreground flex-wrap">
                                  ...
                                  {req.requested_at && (<><span>·</span><span className="flex items-center gap-0.5 text-blue-600 dark:text-blue-400"><Calendar className="w-3 h-3" />Req: {new Date(req.requested_at).toLocaleString('en-IN', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit', hour12: true, timeZone: 'Asia/Kolkata' })} IST</span></>)}
                                  {req.approved_at && (<><span>·</span><span className="flex items-center gap-0.5 text-emerald-600"><CheckCircle className="w-3 h-3" />Approved: {new Date(req.approved_at).toLocaleString('en-IN', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit', hour12: true, timeZone: 'Asia/Kolkata' })} IST{req.approved_by_username ? ` by ${req.approved_by_username}` : ''}</span></>)}
                                  {req.rejected_at && (<><span>·</span><span className="flex items-center gap-0.5 text-red-500"><XCircle className="w-3 h-3" />Rejected: {new Date(req.rejected_at).toLocaleString('en-IN', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit', hour12: true, timeZone: 'Asia/Kolkata' })} IST{req.rejected_by_username ? ` by ${req.rejected_by_username}` : ''}</span></>)}
                                  {req.marked_for_review_at && (<><span>·</span><span className="flex items-center gap-0.5 text-amber-600"><AlertTriangle className="w-3 h-3" />Review: {new Date(req.marked_for_review_at).toLocaleString('en-IN', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit', hour12: true, timeZone: 'Asia/Kolkata' })} IST{req.marked_for_review_by ? ` by ${req.marked_for_review_by}` : ''}</span></>)}
                                </div>
                                {req.offer_stats && (
