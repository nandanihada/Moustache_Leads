  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data.token) {
        ...
        // Store session_id for activity tracking
        if (data.session_id) {
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm">
                <button onClick={() => { setShowForgot(true); setForgotSent(false); setForgotEmail(''); }} className="text-blue-400 hover:text-blue-300 font-medium transition-colors">
                  Forgot password?
                </button>
              </div>
            </div>
            ...
            {/* User Type Toggle - replaces social login */}
            <div className="flex items-center justify-center space-x-4">
              <Link
                to="/publisher/signin"
                className="px-6 py-3 rounded-xl font-semibold transition-all duration-300 bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-lg"
              >
                Publisher
              </Link>
              <Link
                to="/advertiser/signin"
                className="px-6 py-3 rounded-xl font-semibold transition-all duration-300 bg-white/10 border border-white/20 text-gray-300 hover:bg-white/20"
              >
                Advertiser
              </Link>
            </div>
          ...
          <div className="bg-slate-900 border border-white/20 rounded-2xl p-8 w-full max-w-md shadow-2xl">
            {forgotSent ? (
              <div className="text-center space-y-4">
                ...
                <button onClick={() => setShowForgot(false)} className="w-full py-3 bg-gradient-to-r from-blue-500 to-purple-600 text-white font-semibold rounded-xl">
                  Back to Login
                </button>
              </div>
            ) : (
