          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 text-center sm:text-left">
            ...
            <div className="flex flex-col gap-3 mt-4 mb-6">
              <label className="flex items-center gap-2 text-sm text-purple-200 cursor-pointer">
                ...
                <span>I accept the <Link to="/terms" target="_blank" rel="noopener noreferrer" className="text-purple-400 font-semibold underline underline-offset-2 hover:text-white transition-colors" onClick={(e) => e.stopPropagation()}>Terms of Service</Link> and have read the <Link to="/privacy" target="_blank" rel="noopener noreferrer" className="text-purple-400 font-semibold underline underline-offset-2 hover:text-white transition-colors" onClick={(e) => e.stopPropagation()}>Privacy Policy</Link>.</span>
              </label>
            </div>
        ...
        {step === 2 && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="relative mb-8">
              ...
              {/* Selected GEOs Badges */}
              <div className="flex flex-wrap gap-2 mb-3 max-h-36 overflow-y-auto p-1.5 border border-white/5 rounded-lg bg-white/5">
                {formData.geos.map(code => {
                  const country = ALL_COUNTRIES.find(c => c.code === code) || { code, name: code };
                  return (
                    <div
                      key={code}
                      onClick={() => toggleArrayItem('geos', code)}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-green-500/20 border border-green-400/40 text-white text-xs font-semibold rounded-full cursor-pointer hover:bg-green-500/30 hover:border-red-400 hover:text-red-200 transition-all shadow-[0_0_10px_rgba(34,197,94,0.1)] group"
                      title="Click to remove"
                    >
                      <span>{country.name} ({country.code})</span>
                      <X size={12} className="text-green-300 group-hover:text-red-400 transition-colors" />
                    </div>
                  );
                })}
                {formData.geos.length === 0 && (
                  <span className="text-purple-300/40 text-xs italic px-2 py-1">No countries selected yet. Search and select below.</span>
                )}
              </div>
              ...
              <div className="relative">
                ...
                {showGeoDropdown && (
                  <div className="absolute z-50 w-full mt-2 max-h-60 overflow-y-auto bg-slate-900 border border-purple-500/40 rounded-xl shadow-2xl p-2 divide-y divide-white/5 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full">
                    {ALL_COUNTRIES.filter(c => 
                      c.name.toLowerCase().includes(geoSearchQuery.toLowerCase()) || 
                      c.code.toLowerCase().includes(geoSearchQuery.toLowerCase())
                    ).slice(0, 50).map(c => {
                      const isSelected = formData.geos.includes(c.code);
                      return (
                        <div
                          key={c.code}
                          onMouseDown={(e) => {
                          ...
                          onClick={() => {
                            toggleArrayItem('geos', c.code);
                            setGeoSearchQuery("");
                          }}
                          className={`flex items-center justify-between px-4 py-2.5 cursor-pointer text-sm rounded-lg transition-colors ${
                            isSelected 
                        ...
                        >
                          ...
                          {isSelected ? (
                            ...
                            <span className="text-[10px] text-purple-300/40 font-semibold uppercase">{c.code}</span>
                          )}
                        </div>
                      );
                    })}
                    {ALL_COUNTRIES.filter(c => 
                      ...
                      <div className="px-4 py-3 text-sm text-purple-300/40 italic text-center">
