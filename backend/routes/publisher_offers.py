    ...
    try:
        ...
        for offer in offers:
            try:
                ...
                # Calculate publisher payout: use override if set, else 80% of admin payout
                try:
                ...
                if publisher_override and float(publisher_override) > 0:
                    ...
                    # For percentage offers, show 80% of the percentage value
                ...
                offer_data = {
                    ...
                    'category': offer.get('categories', [None])[0] if offer.get('categories') else offer.get('category', 'general'),
                    'vertical': offer.get('categories', [None])[0] if offer.get('categories') else offer.get('vertical', offer.get('category', 'OTHER')),
...
def autocomplete_offers():
    ...
    try:
        ...
        projection = {
            ...
            'payout': 1, 'publisher_payout_override': 1, 'currency': 1, 'status': 1,
        }
        ...
        for offer in top:
            ...
            try:
                _override = offer.get('publisher_payout_override')
                if _override and float(_override) > 0:
                    payout = round(float(_override), 2)
                else:
