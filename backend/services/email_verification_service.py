...
class EmailVerificationService:
    def __init__(self):
        ...
        if self.is_configured:
            logger.info(f"Email service READY")
        else:
    ...
    def generate_verification_token(self, email, user_id):
        ...
        try:
            self.verification_collection.insert_one({'token': token, 'email': email, 'user_id': user_id, 'created_at': datetime.utcnow(), 'expires_at': datetime.utcnow() + timedelta(hours=24), 'verified': False})
            return token
        except:
            return None
