...
# ── MarketXcel Sub-Wall Automation ────────────────────────────────────────────
# Same logic as Voqall but uses import_source='marketxcel'

MARKETXCEL_IMPORT_SOURCE = 'marketxcel'


def run_marketxcel_subwall_automation(db_instance=None) -> dict:
    """
    After each MarketXcel sync:
    1. Find all active MarketXcel offers (import_source='marketxcel').
    2. Rename them to "YIS Survey".
    3. Mark subwall_exclusive=True, show_in_offerwall=False.
    4. Add their offer_ids to the "Moustache Survey's" sub-wall.
    5. Remove paused/stale MarketXcel offers from the sub-wall.
    ...
    if offers_col is None or sub_walls_col is None:
        logger.error('run_marketxcel_subwall_automation: DB collections not available')
        return {'success': False, 'error': 'DB not available', 'renamed': 0, 'added_to_subwall': 0}

    now = datetime.utcnow()

    # ── STEP 1: Find all active MarketXcel offers ─────────────────────────────
    mx_offers = list(offers_col.find(
        {
    ...
    if not mx_offers:
        logger.info('run_marketxcel_subwall_automation: No active MarketXcel offers found')
        return {
            'success': True,
            'message': 'No active MarketXcel offers found',
            'renamed': 0,
            'already_named': 0,
            'added_to_subwall': 0,
            'already_in_subwall': 0,
    ...
    mx_offer_ids = [o['offer_id'] for o in mx_offers if o.get('offer_id')]
    logger.info(f'run_marketxcel_subwall_automation: Processing {len(mx_offer_ids)} offers')

    # ── STEP 2: Rename to "YIS Survey" + mark subwall_exclusive ──────────────
    needs_rename = [
        o for o in mx_offers
        if o.get('name') != YIS_SURVEY_NAME or not o.get('subwall_exclusive')
    ]
    already_named = len(mx_offers) - len(needs_rename)
    ...
    if needs_rename:
        ...
        result = offers_col.update_many(
            ...
            {'$set': {
                ...
                'marketxcel_subwall_tagged': True,
                'marketxcel_subwall_tagged_at': now,
                'updated_at': now,
            }}
        )
        renamed = result.modified_count
        logger.info(f'run_marketxcel_subwall_automation: Renamed {renamed} offers to "{YIS_SURVEY_NAME}"')
    ...
    if not sub_wall:
        logger.warning(
            f'run_marketxcel_subwall_automation: Sub-wall "{TARGET_SUBWALL_SLUG}" not found.'
        )
        return {
            'success': False,
            'error': f'Sub-wall "{TARGET_SUBWALL_SLUG}" not found',
            ...
            'added_to_subwall': 0,
            'already_in_subwall': 0,
            'total_marketxcel_offers': len(mx_offer_ids),
            'run_at': now.isoformat() + 'Z',
        }

    existing_ids_in_wall = set(sub_wall.get('offer_ids', []))
    new_ids = [oid for oid in mx_offer_ids if oid not in existing_ids_in_wall]
    already_in_subwall = len(mx_offer_ids) - len(new_ids)
    ...
    if new_ids:
        ...
        added_to_subwall = len(new_ids)
        logger.info(
            f'run_marketxcel_subwall_automation: Added {added_to_subwall} offers to '
            f'sub-wall "{TARGET_SUBWALL_SLUG}"'
        )

    # ── STEP 4: Remove stale/paused MarketXcel offers from the sub-wall ──────
    ...
    paused_ids = [o['offer_id'] for o in paused_mx if o.get('offer_id')]
    removed_from_wall = 0
    if paused_ids:
        result = sub_walls_col.update_one(
            {'slug': TARGET_SUBWALL_SLUG},
        ...
        if removed_from_wall:
            logger.info(
                f'run_marketxcel_subwall_automation: Removed {len(paused_ids)} stale '
                f'MarketXcel offers from sub-wall'
            )
